...
src/components/
  ui/                                shadcn/ui 基础原语（项目拥有）
  forms/                             可复用、与领域无关的表单组合
  ConfirmAction.tsx                  危险操作确认对话框
  *Form.tsx                          领域表单状态和控件
  *Directory.tsx / ModelOverview.tsx 可交互的公开目录视图
src/lib/
  utils.ts                           shadcn `cn` 辅助函数
  admin/forms/                       FormData -> JSON 负载序列化器
  admin/json-mutation.ts             纯浏览器传输辅助函数
  admin/use-json-mutation.ts         React pending 状态封装
  *-filter.ts                        纯客户端筛选契约
  model-display.ts                   模型展示和筛选辅助函数
```
...
- `src/components/ui/` 仅放 shadcn 基础原语，可按项目需求小幅编辑，但不放业务文案或 API 逻辑。
...
- shadcn 原语：`src/components/ui/button.tsx`、`card.tsx`、`select.tsx`。
...
- 不要把路由目标、删除语义或确认文案隐藏在通用表单原语或 `components/ui` 中。
